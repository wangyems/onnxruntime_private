...
Status SliceOutUnwantedOutputSection(cudaStream_t stream,
  ...
  return ::onnxruntime::cuda::SliceCuda::Impl(stream, input_data, input_dims, output_data, compute_metadata, element_size);
}
...
static size_t GetMaxWorkspaceSize(cudnnHandle_t handle, const ::onnxruntime::cuda::CudnnConvState<cudnnConvolutionFwdAlgoPerf_t>& s,
