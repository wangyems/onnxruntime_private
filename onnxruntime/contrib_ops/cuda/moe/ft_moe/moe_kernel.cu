...
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ < 530
template <typename T, int TPB>
__launch_bounds__(TPB) __global__ void sparse_mixer_top2(const T*, T*, int*, int*, int, float) {
  // Does not support pre-Kepler architectures
  ;
}
#else
...
template <typename T, int TPB>
__launch_bounds__(TPB) __global__
    void sparse_mixer_top2(const T* inputs, T* output, int* indices, int* source_rows, int num_experts, float jitter_eps) {
  using cub_kvp = cub::KeyValuePair<int, T>;
  ...
  for (int k_idx = 0; k_idx < K; ++k_idx) {
    ...
    for (int ii = threadIdx.x; ii < num_experts; ii += TPB) {
      const int idx = thread_row_offset + ii;
      threadData += logits_mask[k_idx] ? 0 : exp((static_cast<float>(inputs[idx]) - float_max));
    }
    ...
    if (threadIdx.x == indices[idx]) {
      ...
      output[idx] =
          logits_mask[k_idx] ? 0 : exp((static_cast<float>(inputs[input_idx]) - float_max)) * normalizing_factor;
    }
  }
}
#endif
...
void topk_gating_softmax_kernelLauncher(const T* input, const bool* finished, T* output, T* softmax_temp_output,
  ...
  if (true) {
    const int TPB = 256;
    sparse_mixer_top2<T, TPB><<<num_rows, TPB, 0, stream>>>(input, output, indices, source_row, num_experts, 0.01f);
...
CutlassMoeFCRunner<T, WeightType, Enable>::CutlassMoeFCRunner(int sm_version, bool has_fc3,
    ...
    : has_fc3_(has_fc3), total_past_rows_(0), total_covered_rows_(0), normalize_routing_weights_(normalize_routing_weights) {
